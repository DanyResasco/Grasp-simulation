...
class BallsStateMachineController(object):
    ...
    def __call__(self,controller):
        ...
        if self.state == 'idle':
            ...
            u = (sim.getTime() - self.t0) / t_grasp
            R_x = so3.from_axis_angle(((0, 1, 0), np.pi))
            goal_angle = so3.mul(self.start_pose[0], R_x)
            goal_pos = deepcopy(self.start_pose[1])
            p, r = get_bb_enclosed_sphere(self.ball)
            goal_pos[0] += r
            goal_pose = (goal_angle, goal_pos)
            T = se3.interpolate(self.start_pose, goal_pose, np.min((u, 1.0)))
            send_moving_base_xform_PID(controller, T[0], T[1])

            if sim.getTime() - self.t0 > t_pregrasp:
    ...
    def _get_hand_pose_for_ball(self, ball):
        p, r = get_bb_enclosed_sphere(ball)
        w_T_op = np.eye(4)
        w_T_op[0,3] = -r
        w_T_op[2,3] =  -r
        """
