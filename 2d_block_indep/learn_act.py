...
activation1 = FullyConnectedLayer(rng, input=input_loc_batch, n_in=4, n_out=num_hidden_layer_neurons)
activation2 = FullyConnectedLayer(rng, input=activation1.output, n_in=num_hidden_layer_neurons, n_out=num_hidden_layer_neurons)
activation3 = FullyConnectedLayer(rng, input=activation2.output, n_in=num_hidden_layer_neurons, n_out=400)

activation_cells = activation3.output

activation_map = activation_cells.reshape((batch_size, 1, 20, 20))

X_activated = input_occ_batch * activation_map

conv1 = ConvLayer(rng, input=X_activated, filter_shape=(nkerns[0], 1, 3, 3), 
...
all_params = activation1.params + activation2.params + activation3.params + conv1.params + conv2.params + fc1.params + fc2.params + output.params
...
while True: # loop through chunk training file
	...
	if chunk_file_idx%100==0:
		...
		save_model(ckpt_name, act1=activation1, act2=activation2, act3=activation3, 
			conv1=conv1, conv2=conv2, fc1=fc1, fc2=fc2, output=output)
