...
if __name__ == '__main__':
    ...
    if dataset == 'balls':
        ...
        from find_index import get_dataset_from_obj_name
        from mvbb.db import MVBBLoader

        db = MVBBLoader()

        only_good_objects = True # TODO Change this back to False for the real task
        rand_g_grasps = []
        g_grasps = db.get_all_scored_poses()
        for gg_obj_name in g_grasps:
            gg_ds = get_dataset_from_obj_name(gg_obj_name)
            rand_g_grasps.append((gg_ds, gg_obj_name))
        if len(sys.argv[2:]) == 0:
            #default: pick 3 random objects
            for i in range(3):
                if only_good_objects:
                    index = random.randint(0,len(rand_g_grasps)-1)
                    shelved.append(rand_g_grasps[index])
                else:
                    dataset = random.choice(objects.keys())
