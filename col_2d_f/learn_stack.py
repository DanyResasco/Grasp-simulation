...
input_occ_batch = X_occ.reshape((batch_size, 3, 100, 100))

print 'defining architecture'

conv1 = ConvLayer(rng, input=input_occ_batch, filter_shape=(nkerns[0], 3, 7, 7), 
	image_shape=(batch_size, 3, 100, 100), poolsize=(2,2))
...
train_set_X_occ, train_set_y, train_set_y_flag = feeder.next_training_set_shared()
...
test_set_X_occ, test_set_y, test_set_y_flag = feeder.test_set_shared()

test_model = theano.function(
	[index], # index can be 0 to 9 because there are 2000 examples in the validation set
	cost,
	givens={
...
while True: # loop through chunk training files
	...
	for i in xrange(50):
		print train_model(i)
	print 'test error %f'%test_model(random.randint(0, 9))
	X_occ_data, y_data, y_flag_data = feeder.next_training_set_raw()
